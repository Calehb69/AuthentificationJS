...
auth.onAuthStateChanged(utilisateur => {
    if(utilisateur){
        info.innerText = "Voici le contenu privé ! Profitez en à bon escient et ouvrez votre esprit !"
        h1.innerText = "Vous voilà de retour ! The game is started :)"
    } else {
        console.log("Utilisateur s'est déconnecté");
        info.innerText = "Contenu public asseptisé et contrôlé, svp merci de ne pas marcher sur la pelouse et surtout restez bien docile.";
        h1.innerText = "Bienvenue, inscrivez-vous et rejoignez Dedsec ! Ou restez dans le troupeau !";
    }
})